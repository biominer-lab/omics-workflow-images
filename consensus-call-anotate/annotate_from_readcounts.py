...
def main():
    ...
    for record in vcf_reader:
        ...
        record.INFO['TumorVAF'] = [tumour_rc.vaf(chrom, pos, alt)]
        record.INFO['NormalVAF'] = [normal_rc.vaf(chrom, pos, alt)]
        record.INFO['TumorVarDepth'] = [tumour_rc.evidence_reads(chrom, pos, alt)]
        record.INFO['TumorTotalDepth'] = [tumour_rc.reads(chrom, pos, alt)]
        record.INFO['NormalVarDepth'] = [normal_rc.evidence_reads(chrom, pos, alt)]
        record.INFO['NormalTotalDepth'] = [normal_rc.reads(chrom, pos, alt)]
...
class BamReadcountFile(object):
    ...
    def evidence_reads(self, chrom, pos, alt):
        ...
        VarDepth = int(forward) + int(reverse)
        return VarDepth
    ...
    def vaf(self, chrom, pos, alt):
        """
        The ratio of alt reads to total reads
        """
        ...
        if int(nreads) == 0:
            ...
            vaf = format(int(VarDepth)*1.0/int(nreads)*1.0, '.5f')
        return vaf
